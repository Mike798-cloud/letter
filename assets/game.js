(() => {
  ...
  const SAVE_KEY = "dead-letter-room-save-v6";
  const LEGACY_KEYS = ["dead-letter-room-save-v5","dead-letter-room-save-v4","dead-letter-room-save-v3","dead-letter-room-save-v2","dead-letter-room-save-v1"];
  ...
  const hints = {
    1:["第一章只要求记录事实。把六个现场热点都看完，不要急着猜凶手。","尸斑、门闩、高窗、通风管、凉茶、后枕伤都需要登记。","终局才会要求把这些事实互相连接。"],
    2:["三封信各有一种物理障碍；先在办公室和配药间找‘方法来源’，再动证物。","压痕需要软芯侧锋；火漆处理法写在死信柜的旧守则里；蓝边纸的采购登记说明它是淀粉上浆纸。","先用面粉做碘反应对照，再把碘酒稀释，用棉签薄涂蓝边纸。"],
    3:["这一章不需要你制造维修零件。维克托只恢复了最低限度气密，你负责验证机械延时。","胶囊是昨夜进入、今晨落下；维修铭牌有立即/3h/6h/隔夜四个卡槽。","把释放销放到隔夜约12小时槽；冷库里再核对层板、拖痕、温度表与笔迹。"],
  ...
  const defaultState = () => ({
    version:6,started:false,chapter:1,scene:"deadroom",clock:460,clues:[],inventory:[],knowledge:[],flags:{},
    ...
    reviewMode:false,reviewVariant:0,reviewNotes:[],deductionStep:0
  });
  ...
  function load(){
    try{
      ...
      state=Object.assign(defaultState(),parsed,{version:6});
  ...
  function renderHeader(){const c=chapters[state.chapter];if(!c)return;els.chapter_kicker.textContent=c.kicker;els.chapter_title.textContent=c.title;els.objective_text.textContent=c.objective;els.objective_progress.textContent=`${progress()} / ${c.target}`;renderClock();}
  ...
  function checkChapter2(){if(["letterA","letterB","letterC"].every(x=>state.flags[x])&&!state.flags.ch2Ready){state.flags.ch2Ready=true;save();queueStory("chapter2");}render();}
  ...
  function checkChapter3(){state.flags.coldSolved=state.clues.includes("c_shelf")&&state.clues.includes("c_drag");if(["tubeSolved","coldSolved","chartSolved","handSolved"].every(x=>state.flags[x])&&!state.flags.ch3Ready){state.flags.ch3Ready=true;unlockFragment(5);save();queueStory("chapter3");}render();}
  ...
  function openLatchReplay(){
    const panel=interactiveShell("现场重演 · 门闩受力路径","重演建立在实际现场照片上。依次标记通风弯头、门闩受力点，再拉动外侧钓线。",`<div class="photo-replay"><img src="assets/images/deadroom.webp" alt="死信室现场复原照片"><div class="replay-mask"></div><button class="photo-pin pin-vent" data-step="vent">1 · 通风弯头</button><button class="photo-pin pin-latch" data-step="latch" disabled>2 · 门闩受力点</button><svg viewBox="0 0 1000 560" preserveAspectRatio="none"><path id="replay-line" d="M735 120 C620 165 410 188 160 285"/></svg><div class="latch-crop" id="latch-crop"><span>插销</span></div></div><label class="range-control">3 · 门外拉力 <input id="replay-range" type="range" min="0" max="100" value="0" disabled><b id="replay-value">0%</b></label><p id="replay-feedback" class="interactive-feedback">先点击照片里的通风弯头。</p>`);
    const vent=panel.querySelector('[data-step="vent"]'),latch=panel.querySelector('[data-step="latch"]'),r=panel.querySelector("#replay-range"),v=panel.querySelector("#replay-value"),fb=panel.querySelector("#replay-feedback"),line=panel.querySelector("#replay-line"),crop=panel.querySelector("#latch-crop");let step=0;
    vent.onclick=()=>{if(step)return;step=1;vent.classList.add("done");latch.disabled=false;latch.classList.add("active");line.classList.add("visible");fb.textContent="现在把线另一端挂到门闩磨痕对应的受力点。";};
    latch.onclick=()=>{if(step!==1)return;step=2;latch.classList.add("done");latch.classList.remove("active");r.disabled=false;fb.textContent="从门外缓慢拉紧。观察插销是否沿磨痕方向移动。";};
    r.oninput=()=>{const n=+r.value;v.textContent=n+"%";crop.style.setProperty("--pull",Math.min(n,90));if(n>=88&&step===2){step=3;fb.innerHTML='<b>受力路径成立。</b> 细线可以绕过弯头，把外侧拉力传到内闩；高窗无需开启。';state.flags.latchReplay=true;save();r.disabled=true;}};
  }
  ...
  function bind(){
    ...
    on(els.archive_btn,"click",openArchive);const archiveForm=$("archive-search");if(archiveForm)archiveForm.onsubmit=e=>{e.preventDefault();archiveSearch();};on(els.notebook_btn,"click",openNotebook);on(els.hint_btn,"click",openHint);on(els.next_hint,"click",nextHint);on(els.support_btn,"click",openSupport);on(els.sound_btn,"click",()=>setSound(!audio.enabled));on(els.menu_btn,"click",()=>openModal("menu-modal"));
